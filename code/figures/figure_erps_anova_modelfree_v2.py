...
for condd, vals in conditions_diff.items():
    ...
    # Drop mastoids to make nicer plot
    mock_nomast = mock.copy().drop_channels(['M1', 'M2'])
    gavg_nomast = gavg_pairs[condd].copy().drop_channels(['M1', 'M2'])
    chankeep =  [True if c not in ['M1', 'M2'] else False for c in mock.ch_names]
    plot_times = [0.6]
    ...
    for t in times_pos:
        ...
        im, _ = mne.viz.plot_topomap(plot_data,
                                    ...
                                    # cmap='viridis',
                                     ...
                                     contours=0,
                                     sensors=True)
        ax.set_title(condd, fontsize=param['titlefontsize']+10)
        plt.tight_layout()
        fig.savefig(opj(outfigpath, 'fig_topo_pairs_' + condd + '.svg'),
                    dpi=600)
...
#     for tidx, t in enumerate(times_pos):
#         plot_data = gavg_pairs[cond].data[:, t] * 1000000  # to get microvolts
#         count = tidx+(cidx*5)
#         im, _ = mne.viz.plot_topomap(plot_data,
...
# Get sd
    ...
    line_axis.legend(by_label.values(), by_label.keys(),
                     loc='upper left', fontsize=param["legendfontsize"]-4,
                     frameon=True)
