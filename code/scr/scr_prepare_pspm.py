...
part = pd.read_csv('/data/source/participants.tsv', sep='\t')

for sub in part.participant_id:

    outpath = opj('/data/derivatives/', sub)
    if not os.path.exists(outpath):
        os.mkdir(outpath)
        os.mkdir(opj(outpath, 'scr'))

    # Load physdata
    phys = pd.read_csv(opj('/data/source', sub, 'eeg',
                           sub + '_task-fearcond_physio.tsv'),
                       sep="\t")

    events = pd.read_csv(opj('/data/source', sub, 'eeg',
    ...
    plt.savefig(opj(opj(outpath, 'scr'), 'import_average.png'), dpi=300)
